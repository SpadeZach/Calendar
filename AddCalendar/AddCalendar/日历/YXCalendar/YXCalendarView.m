...
static CGFloat const yearMonthH = 0;   //年月高度
...
@property (nonatomic, strong) NSString *getYearMonthStr;
@end
...
-(void)setType:(CalendarType)type {
    ...
    if (type == CalendarType_Week) {
        ...
        if (_refreshH) {
            ...
            NSLog(@"%lf",dayCellH + yearMonthH + weeksH);
...
//            }
            ...
            __weak typeof(_scrollV) weakScroll = _scrollV;
            [UIView animateWithDuration:0.3 animations:^{
                weakScroll.frame = CGRectMake(0, yearMonthH + weeksH, ViewW, viewH);
            }];
        }
    }
...
- (void)settingScrollView {
    ...
    _middleView = [[YXMonthView alloc] initWithFrame:CGRectMake(ViewW, 0, ViewW, 270) Date:_currentDate];
    ...
    _rightView = [[YXMonthView alloc] initWithFrame:CGRectMake(ViewW * 2, 0, ViewW, height) Date:_type == CalendarType_Month ? [[YXDateHelpObject manager] getNextMonth:_currentDate] : [[YXDateHelpObject manager]getEarlyOrLaterDate:_currentDate LeadTime:7 Type:2]];
...
- (void)monitorScroll {
    ...
    if (_scrollV.contentOffset.x > 2*ViewW -1) {
        ...
        if (_type == CalendarType_Month) {
            ...
            _rightView.currentDate = [[YXDateHelpObject manager] getNextMonth:_currentDate];
            _rightView.selectDate = _selectDate;
            self.getYearMonthStr = [[YXDateHelpObject manager] getStrFromDateFormat:@"yyyy年MM月" Date:_currentDate];
        } else {
            //下周
...
//代理
- (void)postyearAndMonth{
    __weak typeof(self) weakSelf = self;
    if (weakSelf.getYear) {
        weakSelf.getYear(self.getYearMonthStr);
    }
}
